...
class ConfigGUI(ControlsController.ControlsController):
    ...
    def updateContainer(self):
        self.config_hud_container.set_position((0, Constants.SCREEN_HEIGHT-Constants.HUD_HEIGHT))
        config_hud_container_min_width = self.last_control.get_abs_rect()[0] + self.last_control.get_abs_rect()[2] + Constants.PADDING
        self.config_hud_container.set_dimensions((config_hud_container_min_width, Constants.HUD_HEIGHT))
        self.config_hud_container.set_position(((Constants.SCREEN_WIDTH - config_hud_container_min_width) // 2, Constants.SCREEN_HEIGHT - Constants.HUD_HEIGHT))
    
    def create(self):
        self.config_hud_container = pygame_gui.core.UIContainer( relative_rect=pygame.Rect(0, Constants.SCREEN_HEIGHT-Constants.HUD_HEIGHT, Constants.SCREEN_WIDTH, Constants.HUD_HEIGHT),
                                                            container=self.manager.get_root_container(),
                                                            manager=self.manager)
        self.controls.append(self.config_hud_container)

        # Section HUD Velocity
        self.velocity_name_label = pygame_gui.elements.UILabel( relative_rect=pygame.Rect(10, 10, 90, 20),
                                                                text='Velocity', 
                                                                manager=self.manager,
                                                                container=self.config_hud_container)
